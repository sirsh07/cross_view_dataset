...
def get_parser():
    ...
    parser.add_argument('--megadepth_image_list', required=True)
    parser.add_argument('--output_dir', default='data/megadepth_processed')
    return parser


def main(db_root, megadepth_image_list, output_dir):
    os.makedirs(output_dir, exist_ok=True)

    # Load the megadepth image list
    data = np.load(megadepth_image_list, allow_pickle=True)
    megadepth_data_dict = data['data'].item()

    print('Loading a total of {} scenes, with total of {} images'.format(len(megadepth_data_dict), sum([len(v) for v in megadepth_data_dict.values()])))

    # for each scene, load intrinsics and then parallel crops
    for scene_info, im_names in tqdm(megadepth_data_dict.items(), desc='Overall'):
        scene, subscene = scene_info[0], scene_info[1]
...
if __name__ == '__main__':
    ...
    main(args.megadepth_dir, args.megadepth_image_list, args.output_dir)
