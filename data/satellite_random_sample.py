...
def process_satellite_camera_data(base_path_str, train_size=30, test_size=70):
    base_path = Path(base_path_str)
    id_number = base_path.name.replace("_satellite", "")  # Extract ID like ID0016 from ID0016_satellite

    json_path = base_path / f"{id_number}_satellite.json"
    footage_path = base_path / "footage"

    # Output paths
    train_json_path = base_path / f"{id_number}_satellite_train.json"
    test_json_path = base_path / f"{id_number}_satellite_test.json"
    train_txt_path = base_path / f"{id_number}_satellite_train.txt"
    test_txt_path = base_path / f"{id_number}_satellite_test.txt"
    top_view_path = base_path / f"{id_number}_satellite_top_view.png"
    ...
    ax.set_title(f"{id_number} satelline Camera Poses (Side View)")
    ax.set_xlabel("X")
    ax.set_ylabel("Y")
    ax.legend()
    plt.tight_layout()
...
results = process_satellite_camera_data(
    "./google_earth/ID0016_satellite/"
)
results
