...
from hloc.utils.read_write_model import Camera, Image, Point3D, read_model, write_model, rotmat2qvec, qvec2rotmat, read_cameras_binary, read_images_binary
...
def load_duster_data(colmap_data_folder):
    ...
    # print(f'num_images: {len(images)}')
    # print(f'num_points3D: {len(points3D)}')
    cameras = read_cameras_binary(os.path.join(colmap_data_folder, "cameras" + input_format))
    images = read_images_binary(os.path.join(colmap_data_folder, "images" + input_format))
...
def get_all_dust3r_folders(base_path):
    """
    Recursively finds all 'sparse' directories under the given base COLMAP directory.
...
def main():
    ...
    # else:
    #     master_folders = get_all_mast3r_folders("/home/sirsh/cv_dataset/dataset_30sites/mast3r_sfm")
    ...
    for master_folder in tqdm.tqdm(duster_folders, desc="Processing Master folders"):
        # print(f"Processing Master folder: {master_folder}")
        try:
            mast3r_poses = load_duster_data(master_folder)
        except:
