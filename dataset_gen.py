...
def sample_and_combine_folders_street(base_dir, 
    ...
    for file_path in tqdm(all_sampled_file_path, desc="Symlinking files to 100p folder"):
        file_name = os.path.basename(file_path)
        target_file_path = os.path.join(target_100p, file_name.replace("_street", ""))
    ...
    for file_path in tqdm(split_50p, desc="Symlinking files to 50p folder"):
        file_name = os.path.basename(file_path)
        target_file_path = os.path.join(target_50p, file_name.replace("_street", ""))
        assert os.path.exists(file_path), f"File not found: {file_path}"
        if not os.path.exists(target_file_path):
            os.symlink(file_path, target_file_path)
        original_file_paths.append(file_path)
    ...
    for file_path in tqdm(split_25p, desc="Symlinking files to 25p folder"):
        file_name = os.path.basename(file_path)
        target_file_path = os.path.join(target_25p, file_name.replace("_street", ""))
        assert os.path.exists(file_path), f"File not found: {file_path}"
        if not os.path.exists(target_file_path):
            os.symlink(file_path, target_file_path)
        original_file_paths.append(file_path)
    ...
    for file_path in tqdm(split_12p, desc="Symlinking files to 12p folder"):
        file_name = os.path.basename(file_path)
        target_file_path = os.path.join(target_12p, file_name.replace("_street", ""))
        assert os.path.exists(file_path), f"File not found: {file_path}"
        if not os.path.exists(target_file_path):
            os.symlink(file_path, target_file_path) 
        original_file_paths.append(file_path)
...
def sample_and_combine_folders_satellite(base_dir, 
                               split_folder,
                               target_dir,
                               site_id
                               ):
    ...
    with open(split_file, "r") as f:
        ...
        # sampled_middle_files = random.sample(middle_files, int(len(middle_files) * 1/3))
        middle_files = [
            "_".join([os.path.basename(file).split("_")[0], "satellite"] + os.path.basename(file).split("_")[1:]) 
            if "satellite" not in file else file 
            for file in middle_files
            ]
...
def handle_satellite_data():
    ...
    base_dir = "/home/zhyw86/WorkSpace/google-earth/data/satellite/"
    target_dir = "/home/sirsh/cv_dataset/dataset_10sites/data/satellite/train/"
    split_folder = "/home/zhyw86/WorkSpace/google-earth/sampling/satellite/"
    ...
    for site_id in list((set(os.listdir(base_dir)) & set(os.listdir(split_folder)))-set(os.listdir(target_dir))):
        ...
        site_base_dir = os.path.join(base_dir, site_id)
        site_target_dir = os.path.join(target_dir, site_id.replace("_satellite", "")) 
        site_split_folder = os.path.join(split_folder, site_id)
        
        
        # Sample and combine folders
        sample_and_combine_folders_satellite(
            base_dir=site_base_dir,
            split_folder=site_split_folder,
            target_dir=site_target_dir,
            site_id=site_id.replace("_satellite", "")
        )
